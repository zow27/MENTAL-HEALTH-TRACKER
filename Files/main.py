...
# ─── Journal & Mood Tag ─────────────────────────────────────────────────────────
def verify_user(user_id: int, current_user: models.User):
...
@app.get(
    ...
    tags=["Journal"],
    summary="List all journals for a user"
)
...
@app.get(
    "/motivation/quote",
    tags=["Motivation"],
    summary="Fetch today's motivational quote"
)
def get_motivational_quote():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Could not fetch quote: {e}")
