...
def main(page: ft.Page):
    ...
    for mood in MOOD_OPTIONS:
        btn = ft.ElevatedButton(
            ...
            on_click=lambda e, m=mood: (
                ...
                setattr(e.control.style, "bgcolor", ft.Colors.GREEN_300),
                page.update()
            )
        )
    ...
    def show_user_data(selected_user):
        ...
        external_user_section.controls.append(
            ft.Text(f"📝 {selected_user['username']}'s Journals & Moods", size=52, weight=ft.FontWeight.BOLD)
        )
        try:
            ...
            if rj.status_code == 200:
                for entry in sorted(rj.json(), key=lambda x: x["timestamp"], reverse=True):
                    ...
                    external_user_section.controls.append(ft.Text(f"{date}: {entry['content']}", size=36))
            if rm.status_code == 200:
                for mood in sorted(rm.json(), key=lambda x: x["timestamp"], reverse=True):
                    ...
                    external_user_section.controls.append(
                        ft.Text(f"{date} — Score: {mood['score']}, Category: {mood['category']}", size=36)
                    )
        except Exception as ex:
            external_user_section.controls.append(
        ...
        external_user_section.controls.append(back_button)
        back_button.on_click = back_to_user_list
        page.update()
    ...
    mood_journal_section.controls = [
        ...
        ft.Container(
            ft.ElevatedButton("🚪 Logout", on_click=logout, width=300, height=100, bgcolor=ft.Colors.RED_400),
